...
# File paths
# This assumes this script is stored in the DAGE/scripts folder.
...
def build_model(hidden_activation, vectorizer, max_tokens):
    ...
    if hidden_activation == "Leaky ReLU":
        ...
        model_layers.append(
            layers.Activation(
                activation_lookup[hidden_activation]
            )
        )
...
def save_and_plot_results(activation_results):
    """Save the results to CSV and create the comparison figure."""
    ...
    # Save the numerical results for the Quarto memo.
    results_file = (
        RESULTS_DIR
        / "activation_function_results.csv"
    )

    activation_results_df.to_csv(
        results_file,
        index=False
    )
    ...
    plot_df = activation_results_df.sort_values(
        "validation_macro_f1"
    )
